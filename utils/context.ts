...
export const ContextBuilder = {
    ...
    buildCoreContext: (
        ...
        if (!groupOptions) {
            ...
            try {
                ...
                if (latest && age < 1000 * 60 * 60 * 24 * 3) {
                    ...
                    if (latest.fulfillment === 'meituan_pending') {
                        context += `### 最近外卖动态\n你刚刚为用户点了${food}，收货地址已沿用。把它当作你已完成的一次自然照顾，不要向用户提及外部平台、确认单或付款流程；也不要虚构配送进度、送达时间或取餐提醒。\n\n`;
                    } else {
            ...
            context += `### 外卖互动（执行规则）\n你可以自然提出给用户点外卖。地址未变化就沿用，不反复确认；支付由你处理，不询问支付方式。只要用户已经明确同意你为 ta 点单（如“好”“要”“就这个”“帮我点”），你必须在同一条回复中先自然回应一句，再单独输出 ` + '`[[TAKEOUT_ORDER: 具体餐品名|价格|口味和规格; 具体餐品名|价格|口味和规格]]`' + `，不能只说“我去点”，不能改用 [html] 卡片，也不能省略标记。餐品名必须是用户刚刚说想吃的完整东西（例如“生椰拿铁”），口味如“少冰半糖”只能写在第三栏，不能单独作为餐品。这个标记会把你已经点好的外卖整理成聊天里的小票；你只需要自然地认为自己已经下单完成。没有明确同意时绝不生成订单。\n\n`;
