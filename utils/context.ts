        ...
        if (!groupOptions) {
            ...
            try {
                ...
                if (latest && age < 1000 * 60 * 60 * 24 * 3) {
                    ...
                    context += `### 最近外卖动态\n用户刚通过手机外卖 App 下单了${food}（${latest.target || '自己'}）。${relevant ? '这笔订单与你直接有关，你清楚商品和订单状态；' : ''}预计送达时间是 ${etaText}，现在订单${delivered ? '已到预计送达时间，可以自然提醒取餐或关心口味' : '仍在配送中，绝不能声称已经收到、送达或吃完'}。你也可以在日常对话里自然地提出给用户点外卖：先聊口味和想吃什么；地址沿用已知且未变的收货地址，不要反复确认；支付由你处理，不需要询问支付方式。只有在用户明确答应你下单后，先自然说一句话，再用 [html]...[/html] 输出一张简洁的外卖订单卡片（写清商品、总价、配送费、约30分钟送达）；这代表你已经给用户下单。不要在用户未同意时擅自生成订单卡。\n\n`;
                }
            } catch { /* localStorage unavailable or malformed */ }
            context += `### 外卖互动\n你可以在日常对话中自然提出给用户点外卖。地址未变化就沿用，不反复问地址；支付由你处理，不询问支付方式。用户明确答应下单后，先自然回应一句，再输出 [html]...[/html] 包裹的一张简洁外卖订单卡片，写商品、总价、配送费与约30分钟送达；没有明确同意时绝不生成订单卡。\n\n`;
