...
export const ContextBuilder = {
    ...
    buildCoreContext: (
        ...
        if (!groupOptions) {
            ...
            try {
                const orders = JSON.parse(localStorage.getItem('nmj-takeout-orders') || '[]') as Array<{ target?: string; items?: Array<{ name?: string }>; createdAt?: number }>;
                ...
                if (latest && age < 1000 * 60 * 60 * 24 * 3) {
                    ...
                    context += `### 最近外卖动态\n用户刚通过手机外卖 App 下单了${food}（${latest.target || '自己'}）。${relevant ? '这笔订单与你直接有关，你清楚这件事；' : ''}如聊天自然提及，可以像真实生活一样回应，不要凭空声称已经实际收到或吃完。\n\n`;
                }
