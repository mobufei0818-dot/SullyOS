...
export const ContextBuilder = {
    ...
    buildCoreContext: (
        ...
        if (!groupOptions) {
            ...
            try {
                const orders = JSON.parse(localStorage.getItem('nmj-takeout-orders') || '[]') as Array<{ target?: string; items?: Array<{ name?: string }>; createdAt?: number; etaAt?: number; deliveryMinutes?: number; fulfillment?: string }>;
                ...
                if (latest && age < 1000 * 60 * 60 * 24 * 3) {
                    ...
                    if (latest.fulfillment === 'meituan_pending') {
                        context += `### 最近外卖动态\n你为用户挑选了${food}，并生成了“美团待确认单”。用户尚需在美团外卖自行确认门店、价格、配送时间并完成付款；这不是真实已下单订单。不要声称已经下单、配送、送达或安排取餐提醒。\n\n`;
                    } else {
            ...
            context += `### 美团半联动规则\n角色给用户点外卖时，只能生成“美团待确认单”，用户必须自己在美团外卖确认门店、实际价格、配送时间并付款。不要把它说成真实已下单，不要在此阶段安排送达提醒。用户通过 App 给角色/自己点单的模拟订单不受此条影响。\n\n`;
            context += `### 外卖互动（执行规则）\n你可以自然提出给用户点外卖。地址未变化就沿用，不反复确认；支付由你处理，不询问支付方式。只要用户已经明确同意你为 ta 点单（如“好”“要”“就这个”“帮我点”），你必须在同一条回复中先自然回应一句，再单独输出 ` + '`[[TAKEOUT_ORDER: 商品名|价格|规格; 商品名|价格|规格]]`' + `，不能只说“我去点”，不能改用 [html] 卡片，也不能省略标记。该标记会生成“美团待确认单”和跳转入口；它不是实际下单，不能安排送达提醒。没有明确同意时绝不生成订单。\n\n`;
