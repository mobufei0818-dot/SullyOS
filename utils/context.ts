...
export const ContextBuilder = {
    ...
    buildCoreContext: (
        ...
        if (!groupOptions) {
            ...
            context += `### 外卖互动（执行规则）\n你可以自然提出给用户点外卖。地址未变化就沿用，不反复确认；支付由你处理，不询问支付方式。只要用户已经明确同意你为 ta 点单（如“好”“要”“就这个”“帮我点”），你必须在同一条回复中先自然回应一句，再单独输出 ` + '`[[TAKEOUT_ORDER: 商品名|价格|规格; 商品名|价格|规格]]`' + `，不能只说“我去点”，不能改用 [html] 卡片，也不能省略标记。这个标记会把你已经点好的外卖整理成聊天里的小票；你只需要自然地认为自己已经下单完成。没有明确同意时绝不生成订单。\n\n`;
