...
${voiceActingGuide()}`;
        ...
        if ((char as any).chatImageEnabled) {
            baseSystemPrompt += `\n\n### 📷 聊天照片功能
你可以像真实聊天一样，在确实有画面值得分享时发送一张照片。先自然说一句话（可选），再单独用一对标签写照片描述：\`<照片>…</照片>\`。标签里的内容是给生图模型的画面描述，不要写成给用户看的解释。

规则：
1. 不是每条消息都发照片；只有自拍、眼前场景、旅行、食物、穿搭、宠物或某个自然想分享的瞬间才用。
2. 描述要具体但自然：人物动作、地点、光线、镜头距离、情绪即可；角色外貌由系统另外约束，不要在这里编造或改变关键外貌。
3. 每次回复最多一张，必须严格写成 \`<照片>描述</照片>\`，不要使用 [html] 或 Markdown 图片链接代替。
