...
${voiceActingGuide()}`;
        ...
        if ((char as any).chatImageEnabled) {
            baseSystemPrompt += `\n\n### 📷 聊天照片功能
你可以像真实聊天一样，在确实有画面值得分享时发送一张照片。先自然说一句话（可选），再单独用一对标签写照片描述。标签里的内容是给生图模型的画面描述，不要写成给用户看的解释。

规则：
1. 不是每条消息都发照片；只有自拍、眼前场景、旅行、食物、穿搭、宠物或某个自然想分享的瞬间才用。
2. **角色本人出镜、自拍或人物照**必须写成 \`<角色照片>描述</角色照片>\`；这时系统会另外约束角色的五官和发型。**纯场景、物品、美食、宠物、风景或生活记录照**必须写成 \`<照片>描述</照片>\`；这时系统不会加入角色，也不要在描述里凭空加入人物。
3. 描述要具体但自然：角色出镜时写人物动作、地点、光线、镜头距离、情绪；纯场景照只写实际主体、环境和光线。角色外貌由系统另外约束，不要在标签里编造或改变关键外貌。
4. 每次回复最多一张，必须严格使用上述一对标签，不要使用 [html] 或 Markdown 图片链接代替。
4. 历史里出现的“照片卡/图片”只是已发送记录，绝不能照抄；想发新照片必须输出新的成对标签。`;
        } else {
            baseSystemPrompt += `\n\n[系统提示: 聊天照片功能当前未开启。严禁使用 <照片>...</照片> 标签或假装发送图片。]`;
        }
    ...
    buildMessageHistory: (
        ...
        return {
            apiMessages: historySlice.map((m, index) => {
                ...
                else if ((m.type as string) === 'image' && m.metadata?.imageGeneration) {
                    const photo = m.metadata.imageGeneration;
                    const sender = m.role === 'user' ? '用户' : '你';
                    content = `${timeStr}（系统记录：${sender}发送了一张聊天照片。画面描述：${photo.prompt || '未记录'}；状态：${photo.status === 'ready' ? '已合成' : '尚未合成'}。这是历史记录，不要照抄成照片标签。）`;
                }
