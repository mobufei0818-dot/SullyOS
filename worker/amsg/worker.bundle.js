...
var AMSG_BUNDLE_VERSION = "2026-09-03.r2.moments-runtime-recovery";
...
var handleInstantChat = async (args) => {
  ...
  const fail3 = (status, code, message, extra) => json2(status, { success: false, error: { code, message, ...extra ?? {} } });
...
var WEEKDAY_NAMES2 = ["\u661F\u671F\u65E5", "\u661F\u671F\u4E00", "\u661F\u671F\u4E8C", "\u661F\u671F\u4E09", "\u661F\u671F\u56DB", "\u661F\u671F\u4E94", "\u661F\u671F\u516D"];
...
var describeMomentsLocalTime = (timestamp, timeZone) => {
  if (typeof timeZone === "string" && validTimeZone(timeZone)) {
    ...
    return {
      ...
      weekday: WEEKDAY_NAMES2[new Date(Date.UTC(parts.year, parts.month - 1, parts.day)).getUTCDay()],
      timezone: timeZone,
  ...
  return {
    ...
    weekday: WEEKDAY_NAMES2[shifted.getUTCDay()],
...
var nextMomentsDecisionAt = (actorId, mode, from, timeZone) => {
  ...
  for (let dayOffset = 0; dayOffset < 9; dayOffset += 1) {
    ...
    const dayKey2 = calendar ? `${calendar.getUTCFullYear()}-${padClock(calendar.getUTCMonth() + 1)}-${padClock(calendar.getUTCDate())}` : dateKeyAtTimeZone(localDayStartUtc(from, offsetMinutes) + dayOffset * 24 * 60 * 6e4 + 12 * 60 * 6e4, offsetMinutes);
    if (mode === "low" && stableHash(`${actorId}:${dayKey2}`) % 7 >= 3) continue;
    for (let opportunity = 0; opportunity < windows.length; opportunity += 1) {
      const window2 = windows[opportunity];
      const minuteSpan = window2.span * 60;
      const minute = stableHash(`${actorId}:${dayKey2}:${opportunity}:decision`) % minuteSpan;
      const wallHour = window2.start + Math.floor(minute / 60);
      ...
      if (calendar && ianaZone) {
        ...
        candidate = dayStart + (window2.start * 60 + minute) * 6e4;
      }
...
async function callMomentsModel(credential, prompt, timeoutMs = 9e4) {
  ...
  try {
    const response2 = await fetch(credential.apiUrl, {
      method: "POST",
    ...
    if (!response2.ok) throw new Error(`\u670B\u53CB\u5708\u6A21\u578B HTTP ${response2.status}: ${text.slice(0, 300)}`);
    let data;
    try {
...
var initializeCore = async (target, session, timeoutMs) => {
  ...
  const negotiated = String(
    response2?.result?.protocolVersion || MCP_LATEST_HANDSHAKE_PROTOCOL_VERSION
  );
  ...
  session.serverInfo = response2?.result?.serverInfo || null;
  session.serverCapabilities = response2?.result?.capabilities || null;
...
var createIndexedHourlyCleanupDb = (env) => {
  ...
  const cleanupOutbox = typeof db.cleanupOutbox === "function" ? db.cleanupOutbox.bind(db) : null;
